...
**Restricciones:**
  ...
  - Desigualdad 1: $3 x + 2 y - 120 \leq 0$
  - Desigualdad 2: $x + 2 y - 80 \leq 0$
  - Desigualdad 3: $- x \leq 0$
  - Desigualdad 4: $- y \leq 0$
...
$$\mathcal{L} = \lambda_{0} \left(3 x + 2 y - 120\right) + \lambda_{1} \left(x + 2 y - 80\right) - \lambda_{2} x - \lambda_{3} y - 60 x - 50 y$$

Multiplicadores de desigualdad: $\lambda_{0}$, $\lambda_{1}$, $\lambda_{2}$, $\lambda_{3}$
...
$$\frac{\partial \mathcal{L}}{\partial x} = 3 \lambda_{0} + \lambda_{1} - \lambda_{2} - 60 = 0$$

$$\frac{\partial \mathcal{L}}{\partial y} = 2 \lambda_{0} + 2 \lambda_{1} - \lambda_{3} - 50 = 0$$
...
**Probamos 16 configuraciones posibles de restricciones activas/inactivas:**
...
**Caso 2:**
  - Activas: restricciones 4 → $g_i(x) = 0$
  - Inactivas: restricciones 1, 2, 3 → $\lambda_i = 0$

**Caso 3:**
  - Activas: restricciones 3 → $g_i(x) = 0$
  - Inactivas: restricciones 1, 2, 4 → $\lambda_i = 0$

**Caso 4:**
  - Activas: restricciones 3, 4 → $g_i(x) = 0$
  - Inactivas: restricciones 1, 2 → $\lambda_i = 0$

**Caso 5:**
  - Activas: restricciones 2 → $g_i(x) = 0$
  - Inactivas: restricciones 1, 3, 4 → $\lambda_i = 0$

**Caso 6:**
  - Activas: restricciones 2, 4 → $g_i(x) = 0$
  - Inactivas: restricciones 1, 3 → $\lambda_i = 0$

*(Y 10 casos adicionales...)*
...
- Restricción 1: $3 x + 2 y - 120 = 0$
  - $\lambda_{0} = 35/2$
- Restricción 2: $x + 2 y - 80 = 0$
  - $\lambda_{1} = 15/2$

### Multiplicadores de Lagrange

- $\lambda_{0} = 35/2$ (activa)
- $\lambda_{1} = 15/2$ (activa)
- $\lambda_{2} = 0$ (inactiva)
- $\lambda_{3} = 0$ (inactiva)
...
$$H = \left[\begin{matrix}0 & 0\\0 & 0\end{matrix}\right]$$

**Clasificación:** La Hessiana es *semidefinida positiva*.

**Valores propios:** $\lambda = [0]$

**Interpretación:** La función objetivo es *convexa*.

✓ Como la función es convexa y se cumplen las condiciones KKT, 
el punto hallado es un **máximo global** (puede no ser único).
...
Las restricciones **activas** (que tocan el límite) son: 1, 2

Estas restricciones están **presionando** la solución óptima. Sus multiplicadores λ indican:

- $\lambda_{0} = 35/2$: Sensibilidad del objetivo ante cambios en esta restricción
- $\lambda_{1} = 15/2$: Sensibilidad del objetivo ante cambios en esta restricción
