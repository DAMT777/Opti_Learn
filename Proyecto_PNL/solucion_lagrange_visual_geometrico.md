...
$$f(x, y) = \left(x - 1\right)^{2} + \left(y - 2\right)^{2}$$
...
$$\mathcal{L}(x, y, \lambda) = - \lambda \left(x + 2 y - 4\right) + \left(x - 1\right)^{2} + \left(y - 2\right)^{2}$$

**Componentes:**

- **Función objetivo:** $f(x, y)$
- **Penalización restricción 1:** $-(\lambda \left(x + 2 y - 4\right))$
...
$$\frac{\partial \mathcal{L}}{\partial x} = - \lambda + 2 x - 2 = 0$$

$$\frac{\partial \mathcal{L}}{\partial y} = - 2 \lambda + 2 y - 4 = 0$$

$$\frac{\partial \mathcal{L}}{\partial lambda} = - x - 2 y + 4 = 0$$
...
**Punto óptimo:** $(x^* = 0.8000, y^* = 1.6000)$
...
<img src="/static/tmp/lagrange_5126.png" alt="Visualización de Lagrange" style="max-width: 100%; width: 600px; height: auto; display: block; margin: 20px auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" />
