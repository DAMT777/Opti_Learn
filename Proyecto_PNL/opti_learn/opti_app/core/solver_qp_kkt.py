    ...
    def _generate_explanation(self) -> str:
        ...
        for step in self.steps:
            ...
            if num == 1:
                ...
                lines.append(f"**Condiciones KKT:**")
                lines.append(f"")
                ...
                lines.append(f"  - $\\lambda$ (desigualdades): {contenido.get('n_lambda_ineq', 0)}")
                lines.append(f"  - $\\mu$ (no negatividad): {contenido.get('n_mu', 0)}")
                lines.append(f"")
                lines.append(f"⚠️ **Nota metodológica:**")
                lines.append(f"Este problema se resolverá mediante **SLSQP** (Sequential Least Squares Programming), ")
                lines.append(f"un método numérico moderno que satisface las condiciones KKT al converger. ")
                lines.append(f"NO corresponde al método Simplex de dos fases del libro, sino a un solver ")
                lines.append(f"de optimización no lineal que maneja restricciones de forma eficiente.")
                lines.append(f"")
